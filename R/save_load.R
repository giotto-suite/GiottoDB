...
saveGiotto.GiottoDB <- function(
  ...
  if (dir.exists(final_dir) && overwrite) {
    if (verbose) message("Folder already exists and overwrite = TRUE, overwriting...")
  }
  ...
  if (verbose) message("Preparing spatial objects for save...")
  ...
  for (spat_unit in names(gobject@spatial_info)) {
    if (verbose) message(sprintf("Processing spatial_info: %s", spat_unit))
    
    # Safely get spatVector
    spat_vec <- tryCatch({
      gobject@spatial_info[[spat_unit]]@spatVector
    }, error = function(e) {
      if (verbose) message(sprintf("  Warning: Could not access spatVector for %s: %s", spat_unit, e$message))
      NULL
    })
    ...
    if (inherits(spat_vec, "dbSpatial")) {
      ...
      if (verbose) message(sprintf("  Persisting table %s...", table_name))
    ...
    for (feat_type in names(gobject@expression[[spat_unit]])) {
      for (expr_name in names(gobject@expression[[spat_unit]][[feat_type]])) {
        ...
        if (inherits(expr_obj[], "dbMatrix")) {
          if (verbose) message(sprintf("  Processing expression: [%s][%s][%s]", spat_unit, feat_type, expr_name))
          
          db_mat <- expr_obj[]
          table_name <- paste0("giottodb_expr_", spat_unit, "_", feat_type, "_", expr_name)
          
          if (verbose) message(sprintf("    Persisting view/table %s...", table_name))
  ...
  if (verbose) message("Saving Giotto object structure...")
  ...
  for (backup_key in names(expression_backup)) {
    ...
    gobject@expression[[spat_unit]][[feat_type]][[expr_name]] <- expression_backup[[backup_key]]
  }
  ...
  if (!is.null(db_path_to_move) || db_already_in_place) {
    if (verbose) message("Saving database-backed objects...")
    ...
    if (length(dbspatial_metadata) > 0) {
      if (verbose) message("  Reconnecting dbSpatial objects to new database location...")
...
.loadGiotto.GiottoDB <- function(
  ...
  if (verbose) message("Loading GiottoDB object...")
  ...
  if (verbose) message("Reconnecting to database...")
  ...
  if (dir.exists(db_dir)) {
    ...
    if (exists("db_path")) {
      ...
      if (verbose) message("Reconnecting spatial objects from database...")
      ...
      if (verbose) message("Reconnecting expression matrices from database...")
      ...
      if (file.exists(dbmatrix_metadata_file)) {
        ...
        for (spat_unit in names(dbmatrix_metadata)) {
          for (feat_type in names(dbmatrix_metadata[[spat_unit]])) {
            for (expr_name in names(dbmatrix_metadata[[spat_unit]][[feat_type]])) {
              metadata <- dbmatrix_metadata[[spat_unit]][[feat_type]][[expr_name]]
              table_name <- metadata$table_name
              ...
              if (DBI::dbExistsTable(new_con, table_name)) {
                ...
                if (verbose) {
                  message(sprintf(
                    ...
                    spat_unit, feat_type, expr_name
                  ))
                }
              } else {
                warning(sprintf(
                  "Table '%s' not found in database for expression: [%s][%s][%s]",
                  table_name, spat_unit, feat_type, expr_name
                ), call. = FALSE)
              }
            }
          }
        ...
        if (verbose) {
          message("  No dbMatrix metadata found, skipping expression reconnection")
        }
      }
