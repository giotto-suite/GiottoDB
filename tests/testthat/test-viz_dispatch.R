test_that("Visualization generics exist", {
    ...
    expect_true(is.function(plotUMAP_2D))
    expect_true(is.function(plotPCA_2D))
})
...
test_that("Visualization dispatch works correctly for GiottoDB objects", {
    ...
    expect_error(
        plotUMAP_2D(dummy_db, plot_method = "deckgl"),
        "no applicable method|Package 'rDeckgl' is required|plotUMAP_2D for GiottoDB|Could not retrieve dimension reduction"
    )

    expect_error(
        plotPCA_2D(dummy_db, plot_method = "deckgl"),
        "no applicable method|Package 'rDeckgl' is required|plotPCA_2D for GiottoDB|Could not retrieve dimension reduction"
    )
})
