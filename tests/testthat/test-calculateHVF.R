...
test_that("calculateHVF produces identical HVF column for GiottoDB vs Giotto (cov_groups)", {
  ...
  g_h <- suppressWarnings(Giotto::calculateHVF(g,
    expression_values = "normalized",
    method = "cov_groups",
    show_plot = FALSE, return_plot = FALSE, save_plot = FALSE,
    verbose = FALSE
  ))
  gdb_h <- suppressWarnings(GiottoDB::calculateHVF(gdb,
    expression_values = "normalized",
    method = "cov_groups",
    show_plot = FALSE, return_plot = FALSE, save_plot = FALSE,
    verbose = FALSE
...
test_that("calculateHVF produces equivalent HVF metadata for GiottoDB vs Giotto (var_p_resid)", {
  skip_if_not_installed("GiottoData")
  skip_if_not_installed("dbMatrix")
  skip_if_not_installed("duckdb")
  ...
  expect_equal(fmeta_g$hvf, fmeta_gdb$hvf)
  expect_equal(fmeta_g$var, fmeta_gdb$var, tolerance = 1e-12)
})
...
test_that("calculateHVF.GiottoDB errors on expression_values = 'scaled'", {
  ...
  expect_error(
    GiottoDB::calculateHVF(gdb, expression_values = "scaled"),
