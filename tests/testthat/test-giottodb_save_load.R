...
test_that("GiottoDB save/load works", {
  ...
  expect_true(file.exists(file.path(saved_path, "Database", "dbspatial_metadata.rds")))
  expect_true(file.exists(file.path(saved_path, "Database", "dbmatrix_metadata.rds")))
...
test_that("loadGiottoDB works", {
  ...
  GiottoClass::saveGiotto(gobject, foldername = "test_save_std", dir = save_dir, verbose = FALSE)
